...
status_t VdspIon::mapIonFd(int fd, size_t size, unsigned int memory_type,
                          int uflags) {
  if (Is_ion_legacy(fd)) {
...
int VdspIon::getHeapID() const { return mFD; }

void *VdspIon::getBase() const { return mBase; }

int VdspIon::getIonDeviceFd() const { return mIonDeviceFd; }

int VdspIon::Get_phy_addr_from_ion(int buffer_fd, unsigned long *phy_addr,
                                  size_t *size) {
...
int VdspIon::get_phy_addr_from_ion(unsigned long *phy_addr, size_t *size) {
  if (mIonDeviceFd < 0) {
    ALOGE("%s:open dev ion error!", __func__);
    return MEMION_NO_DEV;
  } else {
...
int VdspIon::Flush_ion_buffer(int buffer_fd, void *v_addr, void *p_addr,
                             size_t size) {
  int fd = open("/dev/ion", O_SYNC);
  if (fd < 0) {
    ALOGE("%s:open dev ion error!", __func__);
...
int VdspIon::Invalid_ion_buffer(int buffer_fd) {
  int fd = open("/dev/ion", O_SYNC);
  if (fd < 0) {
    ALOGE("%s:open dev ion error!", __func__);
    return MEMION_NO_DEV;
...
int VdspIon::flush_ion_buffer(void *v_addr, void *p_addr, size_t size) {
  if (mIonDeviceFd < 0) {
    return MEMION_NO_DEV;
  } else {
    int ret;
...
int VdspIon::invalid_ion_buffer() {
  if (mIonDeviceFd < 0) {
    return MEMION_NO_DEV;
  } else {
    int ret;
...
int VdspIon::Sync_ion_buffer(int buffer_fd) {
  int fd = open("/dev/ion", O_SYNC);
  if (fd < 0) {
    ALOGE("%s:open dev ion error!", __func__);
    return MEMION_NO_DEV;
...
int VdspIon::Get_kaddr(int buffer_fd, uint64_t *kaddr, size_t *size) {
  int fd = open("/dev/ion", O_SYNC);
...
int VdspIon::get_kaddr(uint64_t *kaddr, size_t *size) {
  if (mIonDeviceFd < 0) {
    ALOGE("%s:open dev ion error!", __func__);
    return MEMION_NO_DEV;
  } else {
...
int VdspIon::Free_kaddr(int buffer_fd) {
  int fd = open("/dev/ion", O_SYNC);
